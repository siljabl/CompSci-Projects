...
GENERATOR_OUTPUT_IMAGE_SHAPE = 256 * 256 * 1  #same as real images
...
class amptoph(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    image = Image.open(self.imgfilenames[idx])
...
def make_directory_for_run():
  ...
  if not os.path.exists('./Project3/runs'):
    os.mkdir('./Project3/runs')
  os.mkdir(f'./Project3/runs/{UNIQUE_RUN_ID}')
...
def generate_image(generator, epoch = 0, batch = 0, device=get_device()):
  ...
  for i in range(16):
    ...
    plt.axis('off')
  if not os.path.exists(f'./Project3/runs/{UNIQUE_RUN_ID}/images'):
    os.mkdir(f'./Project3/runs/{UNIQUE_RUN_ID}/images')
  plt.savefig(f'./Project3/runs/{UNIQUE_RUN_ID}/images/epoch{epoch}_batch{batch}.jpg')


def save_models(generator, discriminator, epoch):
  """ Save models at specific point in time. """
  torch.save(generator.state_dict(), f'./Project3/runs/{UNIQUE_RUN_ID}/generator_{epoch}.pth')
  torch.save(discriminator.state_dict(), f'./Project3/runs/{UNIQUE_RUN_ID}/discriminator_{epoch}.pth')
